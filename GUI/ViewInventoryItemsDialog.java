...
public class ViewInventoryItemsDialog extends JDialog {
    ...
    public ViewInventoryItemsDialog(Frame owner, Connection conn) {
    ...
    private void initializeUI() {
        ...
        editIngredientsButton = new JButton("Edit");
    ...
    private void addInventoryItem() {
        ...
        float stock = 0;
        try {
            stock = Float.parseFloat(JOptionPane.showInputDialog(this, "Enter current item stock as an integer:"));
        }
        catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(this, "You must enter an integer number.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        float maxstock = 0;
        try {
            maxstock = Float.parseFloat(JOptionPane.showInputDialog(this, "Enter max item stock as an integer:"));
        }
        catch (Exception e) {
            JOptionPane.showMessageDialog(this, "You must enter an integer number.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        ...
        String units = JOptionPane.showInputDialog(this, "Enter units for stock:");
        if (units == null || units.trim().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Item units are required.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
    ...
    private void deleteMenuItem() {
        ...
        if (selectedRow >= 0) {
            ...
            if (confirmation == JOptionPane.YES_OPTION) {
                int InventoryItemID = InventoryItemIDs.get(selectedRow); // Get the ID from the list
                deleteItemFromDatabase(InventoryItemID);
                refreshTableData();
            }
        } else {
    ...
    public void refreshTableData() {
        ...
        String query = "SELECT * FROM ingredientsinventory;";
        try (Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(query)) {
            while (rs.next()) {
                int id = rs.getInt("ingredientid");
                String name = rs.getString("name");
                float price = rs.getFloat("stock");
                float type = rs.getString("maxstock");
                String type = rs.getString("units");
